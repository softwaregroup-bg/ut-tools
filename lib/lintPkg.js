...
module.exports = async function(from) {
    ...
    function match(parsed, s, name, exp) {
        ...
        } else if (exp === undefined && s === undefined) {
            // do nothing
        } else {
    ...
    async function lintDeps(pkg, parsed) {
        ...
        function checkBlacklist(name, blacklist, whitelist) {
            pkg[name] && Object.keys(pkg[name]).forEach(dep => {
                blacklist.includes(dep) && !whitelist.includes(dep) && error(
        ...
        if (['ut-portal', 'ut-webpack', 'ut-storybook', 'ut-help'].includes(pkg.name)) {
            ...
            // checklist = require('../../ut/ut/node_modules/whitelist/ut/package');
            ...
            checklist = JSON.parse(await request.get('https://raw.githubusercontent.com/softwaregroup-bg/whitelist/master/ut/package.json'));
            // checklist = require('../../ut/ut/node_modules/whitelist/ut/package');
            checkBlacklist('dependencies', Object.keys(checklist.devDependencies), Object.keys(checklist.dependencies));
            ...
            // checklist = require('../../ut/ut/node_modules/whitelist/impl/package');
            checkBlacklist('dependencies', Object.keys(checklist.devDependencies), Object.keys(checklist.dependencies));
            Object.assign(checklist.devDependencies, checklist.dependencies);
    ...
    if (p.scripts) {
        ...
        if (p.scripts.license) match(parsed.scripts.license$location, p.scripts.license, 'scripts.license', /ut-license/);
    ...
    if (p.devDependencies) {
        match(parsed.devDependencies['ut-tools$location'], p.devDependencies['ut-tools'], 'devDependencies.ut-tools', undefined);
    }
