...
module.exports = function(from) {
    ...
    match(p.name, 'name', /^(ut-)|(impl-)|(service-)|(@leveloneproject\/dfsp-)|(@leveloneproject\/ist-)/);
    ...
    const SVCRE = /^git@git\.softwaregroup\.com:dw\/(std|impl\/.+)\/service-.+\.git$/;
    const UTRE = /^git@((git\.softwaregroup\.com:ut5)|(github\.com:softwaregroup-bg))\/ut-.+\.git$/;
    const UTIMPLRE = /^git@git\.softwaregroup\.com:ut5impl\/impl-.+\.git$/;
    const L1PRE = /^git@github.com:LevelOneProject\/((dfsp-)|(ist-)).+\.git$/;
    ...
    if (p.repository) {
        ...
        (typeof p.name === 'string') && p.name.match(/^service-/) && match(p.repository.url, 'repository.url', SVCRE);
