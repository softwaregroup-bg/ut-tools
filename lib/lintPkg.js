...
module.exports = async function(from) {
    ...
    async function lintDeps(pkg) {
        ...
        function checkWhitelist(name, checkUnexpected = true, allowFixed = false) {
            pkg[name] && Object.keys(pkg[name]).forEach(dep => {
                if (checklist[name][dep]) {
                    ...
                    checkUnexpected && error(`Non whitelisted ${dep}@${pkg[name][dep]} in "${name}"`);
                }
            });
        };
        ...
        function checkBlacklist(name, blacklist) {
            pkg[name] && Object.keys(pkg[name]).forEach(dep => {
                blacklist.includes(dep) && error(`Unexpected dev dependency ${dep}@${pkg[name][dep]} found in "${name}"`);
            });
        }

        if (['ut-portal'].includes(pkg.name)) {
    ...
    if (p.scripts) {
        ...
        match(p.scripts.cover, 'scripts.cover', /ut-cover|jest/);
        ...
        match(p.scripts.test, 'scripts.test', /ut-test|jest/);
    }
