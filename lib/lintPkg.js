...
module.exports = async function(from) {
    ...
    async function lintDeps(pkg) {
        ...
        if ((typeof pkg.name === 'string') && pkg.name.match(/^ut-/)) {
            const {ut} = pkg;
            const checkUnexpected = !(ut && ut.whitelist && ut.whitelist.unexpected === false);
            ...
            checkWhitelist('dependencies', checkUnexpected);
            checkWhitelist('devDependencies', checkUnexpected, true);
