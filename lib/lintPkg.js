...
module.exports = function(from) {
    ...
    if (p.repository) {
        ...
        if (typeof p.name === 'string') {
            if (/^(standard|impl-.+)-service/.test(p.name)) match(p.repository.url, 'repository.url', SVCRE);
            ...
            else if (/^@leveloneproject\//.test(p.name)) match(p.repository.url, 'repository.url', L1PRE);
        }
    }
