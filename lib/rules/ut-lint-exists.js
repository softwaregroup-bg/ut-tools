...
function testModulePath(value, fileDir, aliases, extensions, context) {
    ...
    const pkg = getLocalPackageJson(context.getFilename());
...
function testRequirePath(fileName, node, context, config) {
    for (const value of fileName.split('!')) {
        ...
        try {
            const result = testModulePath(value, fileDir, config.aliases, config.extensions, context);
            if (result) {
...
function isInPackage(requirePath, node, context) {
    ...
    const pkg = getLocalPackageJson(context.getFilename());
    if (pkg) {
        const dependencies = getDependencyList(pkg);
        const dependencyToValidate = getDepNameFromRequirePath(requirePath);
...
function getDepNameFromRequirePath(requirePath) {
    if (requirePath.indexOf('/') === -1) {
        ...
        if (isScopedModule(requirePath)) {
            const secondSlashIndex = requirePath.indexOf('/', requirePath.indexOf('/') + 1);
...
const exists = (context) => {
    ...
    const config = {
        extensions: Array.isArray(pluginSettings.extensions) ? pluginSettings.extensions : ['', '.js', '.json', '.node', '.ts', '.tsx'],
...
module.exports = {
    meta: {
        docs: {
            description: 'Check if dependency exists in require paths',
            category: 'Dependencies',
            recommended: false,
            url: 'https://github.com/softwaregroup-bg/ut-tools'
        },
        fixable: 'code',
        schema: [{
            type: 'object',
