...
module.exports = {
    ...
    test: function() {
        if (/^(master|(hotfix|major|minor|patch)\/[^/]+)$/.test(process.env.CIRCLE_BRANCH)) {
            childProcess.spawnSync(`npm run release | tee >(tap-xunit > ${process.env.CIRCLE_TEST_REPORTS}/tape.xml)`, {shell: '/bin/bash', stdio: 'inherit'});
        } else {
            childProcess.spawnSync(`npm run test | tee >(tap-xunit > ${process.env.CIRCLE_TEST_REPORTS}/tape.xml)`, {shell: '/bin/bash', stdio: 'inherit'});
        }
