...
module.exports = {
    ...
    test: function() {
        if (process.env.CIRCLE_BRANCH === 'master') {
            childProcess.execSync(`npm run release | tee (tap-xunit > ${process.env.CIRCLE_TEST_REPORTS}/tape.xml)`);
        } else {
            childProcess.execSync(`npm run test | tee (tap-xunit > ${process.env.CIRCLE_TEST_REPORTS}/tape.xml)`);
        }
