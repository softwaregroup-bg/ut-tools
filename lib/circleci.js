...
module.exports = {
    ...
    test: function() {
        ...
        if (/^(master|(hotfix|major|minor|patch)\/[^/]+)$/.test(process.env.CIRCLE_BRANCH)) {
            childProcess.spawnSync(`set -o pipefail && npm run release | tee >(tap-xunit > ${process.env.CIRCLE_TEST_REPORTS}/reports/tape.xml)`, {shell: '/bin/bash', stdio: 'inherit'});
        } else {
            childProcess.spawnSync(`set -o pipefail && npm run test | tee >(tap-xunit > ${process.env.CIRCLE_TEST_REPORTS}/reports/tape.xml)`, {shell: '/bin/bash', stdio: 'inherit'});
        }
